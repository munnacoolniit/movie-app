...
const Search = ({ placeholder }: propsType) => {
  ...
  useEffect(() => {
    (async () => {
      if (debouncedSearchTerm) {
        ...
        setMovies(
          resp,
          resp?.length ? `Movies found (${resp?.length})` : "No search Result"
        );
      } else {
  ...
  return (
    <div className="w-96 flex items-center bg-white p-2 rounded-full shadow-md">
      <MagnifyingGlassIcon className="w-6 h-6 ml-2 text-gray-500" />
      <input
        type="search"
        onChange={handleChange}
        placeholder={placeholder}
        className="w-full px-4 py-2 rounded-sm outline-none"
      />
    </div>
  );
};
